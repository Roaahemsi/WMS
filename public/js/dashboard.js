document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (userResponse.ok) {
        ...
        document.getElementById('user-name').textContent = userData.username || 'غير متوفر'; // user-id:html , username: server nodejs 
    ...
    // Order Stats Chart
    const orderCanvas = document.getElementById('orderStatsChart'); // 1. اختيار عنصر Canvas: <canvas id="orderStatsChart"></canvas>
    if (!orderCanvas) {
        console.warn('عنصر orderStatsChart غير موجود في DOM');
        displayError('orderStatsChart', 'خطأ: عنصر الرسم البياني للطلبات غير موجود');
    } else if (!statsData.orders || !Array.isArray(statsData.orders)) { // 2. التحقق من صحة البيانات: يتم التحقق من ثلاثة شروط قبل إنشاء المخطط لتجنب الأخطاء:
                                                                       // orderCanvas: يتحقق إذا كان عنصر <canvas> موجودًا.  
                                                                       //statsData.orders: يتحقق إذا كانت البيانات (التي جُلبت من نقطة النهاية /api/supplier-stats) تحتوي على خاصية orders.                                                                         
//Array.isArray(statsData.orders): يتحقق إذا كانت orders عبارة عن مصفوفة (array).
