...
document.addEventListener('DOMContentLoaded', () => {
    fetch('/api/user', {
        ...
        .then(response => {
            if (!response.ok) {
                window.location.href = '/'; // إعادة توجيه إلى تسجيل الدخول
        ...
        .then(data => {
            ...
            if (!['موظف المستودع', 'مدير'].includes(data.role)) {
                ...
                window.location.href = '/login'; // إعادة توجيه إلى تسجيل الدخول
                throw new Error('غير مصرح');
            }

            // إذا كان الدور صالحًا، تابع
        ...
        .catch(error => {
            ...
            // لا حاجة لعرض رسالة خطأ هنا، لأن إعادة التوجيه تحدث أعلاه
        });
});
